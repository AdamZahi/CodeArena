...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, VoiceNavWidgetComponent],
...
export class LevelPlayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.tqService.getMissionById(this.missionId).subscribe({
      next: (mission) => {
        ...
        this.adaptiveService.predictAdaptation(this.userId, this.missionId).subscribe({
          ...
          error: () => { this.isAdaptiveLoading = false; }
        });
      },
      error: () => {
        this.isLoading = false;
    ...
    this.voiceNav.registerPageCommands('level-play', (cmd: string) => {
      if (cmd === 'hint' || cmd === 'indice' || cmd.includes('show hint')) {
        this.showHint = true;
        this.voiceNav.feedback$.next('Hint shown');
        return true;
      }
      if (cmd.includes('back to map') || cmd.includes('retour carte')) {
        this.backToMap();
        return true;
      }
      // Do NOT intercept Linux commands — return false to let accessibility mode handle them
      return false;
    });
  ...
  ngOnDestroy(): void {
    ...
    this.voiceNav.unregisterPageCommands('level-play');
    this.voiceNav.resume();
  }

  // Called by template when accessibility (voice-input) mode is toggled
  onAccessibilityModeChange(active: boolean): void {
    active ? this.voiceNav.pause() : this.voiceNav.resume();
  }
