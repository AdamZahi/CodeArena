...
public interface BattleSubmissionRepository extends JpaRepository<BattleSubmission, UUID> {
    ...
    @Query("SELECT s.language, COUNT(s) FROM BattleSubmission s WHERE s.submittedAt BETWEEN :from AND :to GROUP BY s.language")
    List<Object[]> countGroupedByLanguageBetween(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
