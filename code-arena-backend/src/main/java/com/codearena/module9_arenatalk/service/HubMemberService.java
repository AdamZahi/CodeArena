...
public class HubMemberService {
    ...
    public HubMember joinHub(Long hubId, String auth0Id) {
    ...
    public void leaveHub(Long hubId, String auth0Id) {
        Hub hub = hubRepository.findById(hubId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Hub not found"));
    ...
    public HubMember acceptRequest(Long hubId, Long memberId, String auth0Id) {
        verifyOwner(hubId, auth0Id);
    ...
    public void rejectRequest(Long hubId, Long memberId, String auth0Id) {
        verifyOwner(hubId, auth0Id);
    ...
    public List<HubMember> getPendingRequests(Long hubId, String auth0Id) {
        verifyOwner(hubId, auth0Id);
        return hubMemberRepository.findByHubIdAndStatus(hubId, MemberStatus.PENDING);
    }

    public HubMember setOnline(Long hubId, String auth0Id) {
        HubMember member = hubMemberRepository.findByHubIdAndUserAuth0Id(hubId, auth0Id)
    ...
    public HubMember setOffline(Long hubId, String auth0Id) {
        HubMember member = hubMemberRepository.findByHubIdAndUserAuth0Id(hubId, auth0Id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Member not found"));
        member.setOnline(false);
        member.setLastSeen(LocalDateTime.now());
    ...
    private void verifyOwner(Long hubId, String auth0Id) {
        Hub hub = hubRepository.findById(hubId)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Hub not found"));
        User user = userRepository.findByAuth0Id(auth0Id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found"));
    ...
    public List<Long> getActiveHubIds(String auth0Id) {
        return hubMemberRepository.findActiveHubIdsByAuth0Id(auth0Id);
    }
}